...
public:
	...
	// for CSL debugging
	void addDebugModel(string aircraft, string airline, string livery, double elevOffset, double latOffset, double lonOffset);
	...
	* returns FALSE if the MTL string does not seem to be valid */
	...
	/** create a MTL string from its components.
	* returns FALSE if the components does not seem to be valid */
	static bool createMTL(string aircraft, string airline, string livery, string &outMtl);
	// create a MTL string from string like this: "b738/aal/2".
	static bool createMTL(string mtlWithSlashes, string &outMtl);
...
private:
	...
	//denis maslov 22/08/2017
	// some stuff for correct elevation calculation on the ground.
	static void correctElevOfPosition(bool inIsOnGround, bool inIsClampingIsOn, XPMPPlanePosition_t &inOutPlanePos);

	// debug plane stuff
	XPLMDataRef mDPlaneGearDeploy;

	XPLMDataRef mDPlaneBeaconRef;
	XPLMDataRef mDPlaneLandingRef;
	XPLMDataRef mDPlaneNavRef;
	XPLMDataRef mDPlaneStrobeRef;
	XPLMDataRef mDPlaneTaxiRef;

	XPLMDataRef mDPlaneFlapsDeploy;
	XPLMDataRef mDPlaneSpeedBrakes;

	XPLMDataRef mDPLaneNumEngines;
	// Prop mode: feather=0,normal=1,beta=2,reverse=3
	XPLMDataRef mDPlaneEngineMode;
	// Throttle (per engine) as set by user, 0 = idle, 1 = max
	XPLMDataRef mDPlaneEngineThrust;// float[8]
	XPLMDataRef mDPlaneIsEngineRunning; // bool[8]

	public:
	std::map<std::string, std::pair<bool, double> > mDPlaneAnimOvverides;
	private:
